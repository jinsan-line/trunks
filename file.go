...
func decoder(files []string) (trunks.Decoder, io.Closer, error) {
	closer := make(multiCloser, 0, len(files))
	decs := make([]trunks.Decoder, 0, len(files))
	for _, f := range files {
		rc, err := file(f, false)
		if err != nil {
	...
	return trunks.NewRoundRobinDecoder(decs...), closer, nil
}
