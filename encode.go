...
const encodeUsage = `Usage: trunks encode [options] [<file>...]

Encodes trunks attack results from one encoding to another.
...
Examples:
  echo "GET http://:80" | trunks attack -rate=1/s > results.gob
  cat results.gob | trunks encode | jq -c 'del(.body)' | trunks encode -to gob
`

func encodeCmd() command {
	encs := "[" + strings.Join([]string{encodingCSV, encodingGob, encodingJSON}, ", ") + "]"
	fs := flag.NewFlagSet("trunks encode", flag.ExitOnError)
